...
import { getCmsPageDraftFn } from "@/server/phase2/fns";
...
export const Route = createFileRoute("/admin/content/$slug/preview")({
  loader: async ({ params }) => {
    ...
    if (!r.ok) return { error: r.error, page: null as null };
    return { error: null as string | null, page: r.data };
  },
  head: ({ loaderData }) => ({
...
function DraftPreview() {
  ...
  const { error, page } = Route.useLoaderData();
