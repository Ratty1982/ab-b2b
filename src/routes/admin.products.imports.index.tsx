        ...
        actions={
          <div className="flex gap-2">
            ...
            <button
              type="button"
              disabled={uploading}
              className="h-10 rounded-md bg-primary px-5 text-[13px] font-bold uppercase text-primary-foreground disabled:opacity-50"
      ...
      <div className="p-4 sm:p-6">
        ...
        <div className="overflow-x-auto rounded-lg border border-border">
          <table className="w-full min-w-[720px] text-[13px]">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={9} className="px-3 py-10 text-center text-sm text-steel">No import jobs yet.</td></tr>
              ) : rows.map((row, i) => (
                <tr key={row.id} className={cn("border-b border-border/60", i % 2 && "bg-surface/30")}>
                  ...
                  <td className="px-3 py-2 text-right">
                    <Link
                      to="/admin/products/imports/$id"
                      params={{ id: row.id }}
                      className="inline-flex h-8 items-center rounded-md border border-border px-3 text-[11px] font-semibold uppercase text-ink"
                    >
                      {row.status === "APPLIED" || row.status === "FAILED" ? "View" : "Continue"}
                    </Link>
                  </td>
                </tr>
              ))}
